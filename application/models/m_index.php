...
//                                if($j == 1)
    ...
    public function getCommentStr(){
        ...
        try{
            ...
            $sql_creadby = "SELECT a.*, b.`FullName`,c.`URL` 
                            FROM `users` a  
                            INNER JOIN `objects` b ON a.`ObjectId` = b.`ObjectId`
                            LEFT JOIN `objectlinks` c ON c.`ObjectIDD` = b.`ObjectId`
                            WHERE c.`Type` = 'USER'";
                    ...
                    if(count($list_creadbyComment) > 0){
                       for($j = 0; $j < count($list_creadbyComment); $j++){
                            if($list_creadbyComment[$j]->UserId == $list[$i]->CreatedBy){
                                $image = $list_creadbyComment[$j]->URL;
                                if($image == ""){
