...
class M_user extends CI_Model
{
    ...
    public function xoauser()
    {
        ...
        $sql_xoalinkshinh1          =   "select * from `$db1`.`links` where ObjectIDD = '".$UserId."'";
        $query_xoalinkshinh1        =   $this->db->query($sql_xoalinkshinh1)->result();
        
        //print_r($query_xoalinkshinh);die;
        if(count($query_xoalinkshinh1)>0)
        {
            foreach($query_xoalinkshinh1 as $row1)
            {
                $url                =   $row1->URL;
            	 //echo $url;
                 unlink($url);
            }      
            $sql_xoatbl_link1       =   "delete from `$db1`.`links` where ObjectIDD = '".$UserId."'";
            $query_xoatbl_link1     =   $this->db->query($sql_xoatbl_link1);
        }
        else
        {
            $query_xoatbl_link1     =    true;
        }
        
        $sql_obj1                    =   "DELETE FROM `$db1`.`objects` WHERE `objects`.`ObjectId` = '$ObjectId'";
        $res_obj1                    =   $this->db2->query($sql_obj1);
        $sql_obj2                    =   "DELETE FROM `$db2`.`objects` WHERE `objects`.`ObjectId` = '$ObjectId'";
        $res_obj2                    =   $this->db2->query($sql_obj2);
        
        $sql_user1                  =   "DELETE FROM `$db1`.`users` WHERE `UserId`='$UserId'";
        $res_user1                  =   $this->db2->query($sql_user1);
        $sql_user2                  =   "DELETE FROM `$db2`.`users` WHERE `UserId`='$UserId'";
        $res_user2                  =   $this->db2->query($sql_user2);
        
        if($res_obj1 && $res_obj2 && $res_user1 && $res_user2 && $query_xoatbl_link1 )
